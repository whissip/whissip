...
function _add_token($type, $data, $do = false) {
    if($this->get_cfg('preserve_css') || $do) {
        $this->tokens[] = array($type, trim($data)); // dh> changed from upstream: also trim COMMENT
    }
}
...
function _unicode(&$string, &$i)
{
	...
	while($i < $n)
	{
		...
		if((ctype_xdigit($cur_char) || ctype_space($cur_char)) && $len_add < 6)
		{
			$add .= $cur_char;
			$len_add = strlen($add);

			if(ctype_space($cur_char)) {
				break;
			}
			$i++;
		}
	...
	if(@ctype_xdigit(substr($string, $i+1, 1)) && ctype_space(substr($string, $i, 1))
       && !$replaced || !ctype_space(substr($string, $i, 1))) {
		$i--;
	}

	if($add !== '\\' || !$this->get_cfg('remove_bslash') || strpos($tokens, substr($string, $i+1, 1)) !== false) {
		return $add;
	}
...
{
	$tokens =& $GLOBALS['csstidy']['tokens'];
	return (strpos($tokens, substr($string, $i, 1)) !== false && !csstidy::escaped($string,$i));
}
...
function parse($string) {
    ...
    for ($i = 0, $size = strlen($string); $i < $size; $i++ )
    {
        ...
        switch($this->status)
        {
            ...
            {
                if($cur_char === '/' && substr($string, $i+1, 1) === '*')
                {
                    $this->status = 'ic'; ++$i;
                    $this->from = 'at';
                }
            ...
            else
            {
                ...
                if(!( (ctype_space($this->at{$lastpos}) || csstidy::is_token($this->at,$lastpos) && $this->at{$lastpos} === ',') && ctype_space($cur_char)))
                {
                    $this->at .= $cur_char;
                }
            }
            ...
            if(csstidy::is_token($string,$i))
            {
                if($cur_char === '/' && substr($string, $i+1, 1) === '*'
                  && ( trim($this->selector) == ''
                    || preg_match('~^.*?\*/\s*[,{]~s', substr($string, $i+2)) // comment gets followed by comma or curly brace
                    || preg_match('~,\s*$~', substr($string, 0, $i-1)) // comment follows a comma
                  )) {
                    $this->status = 'ic'; ++$i;
                    $this->from = 'is';
                } elseif($cur_char === '@' && trim($this->selector) == '') {
                    // Check for at-rule
                    $this->invalid_at = true;
                    foreach($at_rules as $name => $type)
                    {
                        $len_name = strlen($name);
                        if(!strcasecmp(substr($string,$i+1,$len_name),$name))
                        {
                ...
                } elseif(($cur_char === '"' || $cur_char === "'")) {
                    $this->cur_string = $cur_char;
                    $this->status = 'instr';
                    $this->str_char = $cur_char;
                    $this->from = 'is';
					/* fixing CSS3 attribute selectors, i.e. a[href$=".mp3" */
					$this->quoted_string = ($string{$i-1} == '=' );
                } elseif($this->invalid_at && $cur_char === ';') {
                    $this->invalid_at = false;
                    $this->status = 'is';
                } elseif($cur_char === '{') {
                    $this->status = 'ip';
                ...
                } elseif($cur_char === '*' && @in_array(substr($string, $i+1, 1), array('.', '#', '[', ':'))) {
                    // remove unnecessary universal selector, FS#147
                } else {
                    $this->selector .= $cur_char;
                }
            ...
            else
            {
                ...
                if($lastpos == -1 || !( (ctype_space($this->selector{$lastpos}) || csstidy::is_token($this->selector,$lastpos) && $this->selector{$lastpos} === ',') && ctype_space($cur_char)))
                {
                    $this->selector .= $cur_char;
                }
            }
            ...
            if(csstidy::is_token($string,$i))
            {
                if(($cur_char === ':' || $cur_char === '=') && $this->property != '')
                {
                    $this->status = 'iv';
                ...
                elseif($cur_char === '/' && substr($string, $i+1, 1) === '*' && $this->property == '')
                {
                    $this->status = 'ic'; ++$i;
                    $this->from = 'ip';
                }
            ...
            elseif(!ctype_space($cur_char))
            {
                $this->property .= $cur_char;
            }
            break;

            /* Case in-value */
            case 'iv':
            $pn = (($cur_char === "\n" || $cur_char === "\r") && $this->property_is_next($string,$i+1) || $i == strlen($string)-1);
            if(csstidy::is_token($string,$i) || $pn)
            {
                ...
                {
                    $this->cur_string = $cur_char;
                    $this->str_char = ($cur_char === '(') ? ')' : $cur_char;
                    $this->status = 'instr';
                    $this->from = 'iv';
                }
                elseif($cur_char === ',')
                ...
                elseif($cur_char !== '}')
                {
                    $this->sub_value .= $cur_char;
                }
                if(($cur_char === '}' || $cur_char === ';' || $pn) && !empty($this->selector))
                {
                    if($this->at == '')
                    {
                        $this->at = DEFAULT_AT;
            ...
            /* Case in string */
            case 'instr':
            if($this->str_char === ')' && ($cur_char === '"' || $cur_char === '\'') && !$this->str_in_str && !csstidy::escaped($string,$i))
            {
                $this->str_in_str = true;
            }
            elseif($this->str_char === ')' && ($cur_char === '"' || $cur_char === '\'') && $this->str_in_str && !csstidy::escaped($string,$i))
            {
                $this->str_in_str = false;
            }
            $temp_add = $cur_char;           // ...and no not-escaped backslash at the previous position
            if( ($cur_char === "\n" || $cur_char === "\r") && !($string{$i-1} === '\\' && !csstidy::escaped($string,$i-1)) )
            {
                $temp_add = "\\A ";
                $this->log('Fixed incorrect newline in string','Warning');
            }
            if (!($this->str_char === ')' && in_array($cur_char, $GLOBALS['csstidy']['whitespace']) && !$this->str_in_str && !csstidy::escaped($string, $i))) {
                $this->cur_string .= $temp_add;
            }
            if($cur_char == $this->str_char && !csstidy::escaped($string,$i) && !$this->str_in_str)
            {
